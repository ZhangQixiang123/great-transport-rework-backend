...
from app.description import generate_description, translate_title
...
async def main():
    ...
    try:
        ...
        if not validated_queries:
            print(f"\n  All queries were saturated! Nothing to search on YouTube.")
            print(f"  Try running again — the LLM may generate different queries.")
            return
...
""")
        ...
        # Show scoring breakdown for top N
        top_n = min(args.top_n, len(scored))
        print_section(f"Detailed Scoring Breakdown (top {top_n})")
        for rank, (c, score) in enumerate(scored[:top_n], 1):
            like_ratio = c.likes / max(c.views, 1)
        ...
        if args.upload or args.dry_run:
            ...
            for rank, (c, score, tr) in enumerate(transport_results, 1):
                ...
                try:
                    chinese_title = translate_title(backend, c.title)
                    llm_call_count += 1
                    if chinese_title != c.title:
                        print(f"    Title translated: {chinese_title}")
                    else:
                        print(f"    Title unchanged (translation returned original)")
                except Exception as e:
                    print(f"    WARNING: Title translation failed: {e}")
                ...
                try:
                    ...
                    desc = generate_description(backend, video_info)
                    llm_call_count += 1
